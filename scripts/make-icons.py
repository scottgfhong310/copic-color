...
icon 的概念：**三乘三色票矩陣**——把本 app 的招牌縮成一枚標記。
三欄＝色系（RV／YG／B），三列＝Intensity（淺 → 深），正是 Copic Color System
那兩個軸；`copic-color` 的主頁就是這個矩陣的放大版。
與家族其他色彩 app 清楚區隔（faber-castell-color＝橫向色帶的平面色卡、
caran-dache-color＝色卡扇、thangka-trace＝三欄對照）。
**刻意不用 COPIC 的品牌 logo**——避免冒用商標，標記只描述資料的形狀。

**九格全部是真實 COPIC 色，且由 `data/copic-colors.js` 現查**（不寫死 hex）：
...
APP = os.path.join(os.path.dirname(__file__), '..', 'public', 'apps', 'copic-color')
OUT = os.path.join(APP, 'icons')
DATA = os.path.join(APP, 'data', 'copic-colors.js')
...
def hexes():
    """由 data/copic-colors.js 取九格的 hex；缺任何一個就拋錯。"""
...
def build_svgs(fills):
    ...
    files = {
        'copic-color-icon.svg':       tile(512, m_d, DARK_TILE),
        'copic-color-icon-light.svg': tile(512, m_l, LIGHT_TILE, True),
...
def build_manifest():
    m = {
        "name": "copic-color",
        "short_name": "COPIC colour",
        "description": "COPIC colour code → CSS reference, browsed along the "
                       "Copic Color System's three axes.",
        "start_url": "/apps/copic-color/",
        "scope": "/apps/copic-color/",
        "display": "standalone",
